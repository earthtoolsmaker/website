...
tagline: Recognising individual trout by their spot patterns, as unique as fingerprints, without ever touching the fish.
clients:
  - name: Lumax AI
    link: https://lumax.ai/
...
straightened into a standard pose. Its unique spots are then extracted as
...
strong enough match returns that individual, along with its PIT tag and name; a weak
match means it is a fish we have not seen before, ready to be registered.

{{< gallery caption="A genuine match (left), where the spot keypoints line up across two photos of the same fish, and a non-match (right), where they don't." >}}
