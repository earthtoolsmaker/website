...
Here are all three passes acting on a real sequence, where each row is a
candidate tube and each dot a detection:
...
The tracker initially sees one plume as three fragments because the detector
...
checkpoint, the calibrator coefficients, and the full configuration (every
threshold and parameter named in this post) travel together in a
